...
export default function ManageAgencies({ sidebarOpen, setSidebarOpen }) {
    ...
    useEffect(() => {
        axios
            .get("http://localhost:5002/travelAgencies")
    ...
    const handleDelete = (id) => {
        ...
        axios
            .delete(`http://localhost:5002/travelAgencies/${id}`)
            .then(() => {
                setAgencies(agencies.filter((item) => item.id !== id));
            })
            .catch((err) => console.log(err));
    ...
    const changeStatus = (id, newStatus) => {
        axios
            .patch(`http://localhost:5002/travelAgencies/${id}`, {
                status: newStatus,
            })
            .then(() => {
                setAgencies(
    ...
    const filteredData =
        activeTab === "all"
            ? agencies
            : agencies.filter((item) => item.status === activeTab);
